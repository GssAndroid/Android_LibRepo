...
public class PriceListActivity extends ListActivity {
	...
	private TextView custNameTV, myTitle;
	...
	private String taskErrorMsgStr = "", custSearchStr, CustomerStr, custname = "", Custnumb = "", title="";
	...
	private static boolean chckIntrntConn;
    ...
    private OnClickListener Customersrch_btnListener = new OnClickListener() {
        public void onClick(View v) {        	
        	if(custSearchET.getText().toString().trim().length() != 0) {
        		...
        		SalesOrderProConstants.showErrorDialog(PriceListActivity.this, "Enter Material name to search.");
        }
    };
    
    public void initCustomerDBConnection(String CustmerSrchStr){
    	CustArrayList = SalesOrderDBOperations.readAllCustomerIdDataFromDB(PriceListActivity.this,CustmerSrchStr);
    ...
    private void getCustomerSearch(String custIdStr){
    	try{
			...
			isConnAvail = SapGenConstants.checkConnectivityAvailable(PriceListActivity.this);//check for internet connection			
    ...
    private void showPriceListDetailScreen(){
    	try{
    		CustomerStr = custSearchET.getText().toString().trim();
    		Intent intent = new Intent(this, PriceListDetailScreen.class);
    ...
    private OnClickListener showCustBtnListener = new OnClickListener(){
        public void onClick(View v) {        	
	    	...
	    	if(priceArrSelctdList.size() > 0){
	    		...
	    		SalesOrderProConstants.showErrorDialog(PriceListActivity.this, "Material Id is empty!");
	     }
    };
        ...
        public void onClick(View v) {        	
        	if(customerET.getText().toString().trim().length() != 0) {
				try{
					...
					SalesOrderProConstants.showLog("customer : "+custSearchStr);
					//new	
					isConnAvail = SalesOrderProConstants.checkConnectivityAvailable(PriceListActivity.this);//check for internet connection					
					if(isConnAvail!=false)
    ...
    private void initDBConnection(String idStr) {    	
		priceList = PriceListDBOperations.readAllSerchIdDataFromDB(PriceListActivity.this,idStr);
		SalesOrderProConstants.showLog("info array size : "+priceList.size());	
		if(priceList.size()!=0)
			prefillPriceListData(); 		
		else {
			SalesOrderProConstants.showErrorDialog(PriceListActivity.this, "Data Not available in Offline mode");
		}	
    }//initDBConnection
    ...
    public void updateCustomerServerResponse(SoapObject soap){		
        try{ 
	                        ...
	                        if(j > 2){
	                            ...
	                            if(errorFstIndex > 0){
                                    ...
                                    this.runOnUiThread(new Runnable() {
                                        public void run() {
                                        	SalesOrderProConstants.showErrorDialog(PriceListActivity.this, taskErrorMsgStr);
                                        }
                                    });
	                            }
	                        }
        	...
        	this.runOnUiThread(new Runnable() {
                public void run() {
                	prefillcustomerData();
                	SalesOrderDBOperations.deleteAllTableDataFromDB(PriceListActivity.this, SalesOrderCP.SO_CUST_CONTENT_URI);
   ...
   public void getCustomerString(int pos){
    	 ...
    	 */
    	 ...
    	 if(headerLinear != null)
    		 headerLinear.setVisibility(View.VISIBLE);   	 
    	 if(custNameTV != null && (custname.length()<=16  && Custnumb.length()<10)){
 			custNameTV.setText(": "+custname+" "+"("+Custnumb+")");
 			}
 		else{
 			String upToNCharacters = custname.substring(0, Math.min(custname.length(),12));
 			custNameTV.setText(": "+upToNCharacters+".."+"("+Custnumb+")");
 			SalesOrderProConstants.showLog("Size of custname : "+custname.length()); 			
 		}    	
    }//  
    	...
    	try {
			if(CustArrayList != null){
				for(int k=0; k<CustArrayList.size(); k++){
					...
					if(custCategory != null)
						SalesOrderDBOperations.insertCustomerDataInToDB(PriceListActivity.this,custCategory);
				}
			}
		} catch (Exception ewe) {
   ...
   public void initDBConnection(){
	   try {
		   priceList = PriceListDBOperations.readAllSerchDataFromDB(PriceListActivity.this);
			SalesOrderProConstants.showLog("info array size : "+priceList.size());
