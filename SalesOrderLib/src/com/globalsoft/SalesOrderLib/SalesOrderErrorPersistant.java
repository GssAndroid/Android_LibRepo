...
public class SalesOrderErrorPersistant {
    ...
    public SalesOrderErrorPersistant(Context ctext){
    	context = ctext;    
    }
    ...
    public void closeDBHelper(){
    	try{
			...
			SapGenConstants.showErrorLog("Error in ServiceProErrorMessagePersistent closeDBHelper:"+e.toString());
		}
    }//fn closeDBHelper       
    ...
    /*public ArrayList checkErrorTrancIdApiExists(){
    	...
    	try {
			DBHelper = new DatabaseHelper(context);
			sqlitedatabase = DBHelper.getWritableDatabase();
			c = sqlitedatabase.rawQuery("select * from "+DATABASE_TABLE+" where "+KEY_APINAME+"='"+SapGenConstants.STATUS_UPDATE_API+"' or "+KEY_APINAME+"='"+SapGenConstants.TASK_CONFIRMATION_API+"' or "+KEY_APINAME+"='"+SapGenConstants.TASK_TRANS_API+"'", null);
			//SapGenConstants.showLog("select * from "+DATABASE_TABLE+" where "+KEY_APINAME+"='"+SapGenConstants.STATUS_UPDATE_API+"' or "+KEY_APINAME+"='"+SapGenConstants.TASK_CONFIRMATION_API+"' or "+KEY_APINAME+"='"+SapGenConstants.TASK_TRANS_API+"'");
			if (c.moveToFirst())
			{
			     do {
					 int index = c.getColumnIndex(KEY_TRANCID);
					 String tranc_no = c.getString(index);					 
					 //SapGenConstants.showLog("tranc_no:"+tranc_no);
			...
			SapGenConstants.showErrorLog("Error in checkErrorTrancIdApiExists:"+e.toString());
		}   
...
*/    
    public boolean checkTrancIdApiExists(String id, String apiName){
    	...
    	try {
			...
			//SapGenConstants.showLog("select * from "+DATABASE_TABLE+" where "+KEY_TRANCID+"="+idValue+" and "+KEY_APINAME+"='"+apiName+"'");
			//SapGenConstants.showLog("getCount:"+c.getCount());
			if (c.moveToFirst())
			{
			     do {
					 int index = c.getColumnIndex(KEY_TRANCID);
					 ...
					 /*SapGenConstants.showLog("tranc_no:"+tranc_no);
					 SapGenConstants.showLog("apiNameStr"+apiNameStr);*/
		     		 if(id.equals(tranc_no) && apiName.equals(apiNameStr)){
		     			 idexists = true;
		     			 //SapGenConstants.showLog("Id value:"+c.getInt(index));
     				 	 sqlitedatabase.close();
     				 	 return idexists;
		     		 }
			     } while (c.moveToNext());
			...
			SapGenConstants.showErrorLog("Error in checkTrancIdApiExists:"+e.toString());
		}   
    	return idexists;
    ...
    {   	
    	...
    	try {
    		...
    		SapGenConstants.showErrorLog("Error in getIdList:"+e.toString());
			sqlitedatabase.close();
		}
		return resArray;   
    }//fn getIdList 
    ...
    {   	
    	try {
    		...
    		SapGenConstants.showErrorLog("Error in deleteRowForStatus:"+e.toString());
			sqlitedatabase.close();
		}   
