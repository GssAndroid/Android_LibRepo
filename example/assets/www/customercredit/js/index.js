		...
		.each(function()
			{
				...
				if(i == 2)
				{
					...
					whereColumns["value"] = customer_search;
...
function customerSearch()
{
	...
	dropDynamicTable(window.localStorage.getItem(customer_search));
		window.localStorage.removeItem(customer_search);
...
function getOfflineValues(tx) {    	
	tx.executeSql('SELECT * FROM FUNCTIONMASTER WHERE id = '+customer_search, [], showquerySuccess, errorCB);
}
