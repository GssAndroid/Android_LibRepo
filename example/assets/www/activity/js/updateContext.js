...
function getScreenNameAddActivity(results)
{
	...
	for(var i = 0; i < len; i++)
	{
		...
		$("#screenTitle").text(fieldLabelRow.VALUE + updateActivityId);
	}
}
...
function showqueryContextActivitySuccess(tx, results) 
{
	...
	if(tname != "")
	{
		...
		query = 'SELECT * FROM ' + tname +' WHERE CNTXT4 = "SCRN-TITLE" AND CNTXT2 = "UPDATE-ACTIVITY-W"';
		selectTableValues(query,getScreenNameAddActivity);
		
		
		query = 'SELECT * FROM ' + tname +' WHERE CNTXT4 = "FIELD-ATTR" AND CNTXT2 = "UPDATE-ACTIVITY-W-MAIN-BLOCK" ORDER BY SEQNR ASC';
		selectTableValues(query,getFieldAttr);
		
		var query1 = 'SELECT * FROM ' + tname +' WHERE CNTXT2 = "UPDATE-ACTIVITY-W-MAIN-BLOCK" AND (CNTXT4 = "FIELD-LABE" OR CNTXT4 = "FIELD-HINT") ';
