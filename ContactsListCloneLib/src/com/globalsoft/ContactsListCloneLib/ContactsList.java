...
import com.globalsoft.ContactsListCloneLib.Constraints.ContactProContactCreationKeyOpConstraints;
import com.globalsoft.ContactsListCloneLib.Constraints.ContactProContactCreationOpConstraints;
import com.globalsoft.ContactsListCloneLib.Constraints.ContactProInputConstraints;
import com.globalsoft.ContactsListCloneLib.Database.ContactProSAPCPersistent;
...
public class ContactsList extends ListActivity implements TextWatcher {
	...
	private ArrayList contactsArrySelctdList = new ArrayList();
    ...
    private SalesProActOutputConstraints categoryAct = null;
    ...
    public void onCreate(Bundle savedInstanceState) {
		...
		categoryAct = (SalesProActOutputConstraints)this.getIntent().getSerializableExtra("actobj");
            ...
            if((allContactVect != null) && (allContactVect.size() > 0)){
                if((!match.equalsIgnoreCase("")) && (match.length() >= 1)){             
                    ...
                    for(int i = 0; i < allContactVect.size(); i++){ 
        				...
        				//String cusid = (String)((ContactSAPDetails)contactVect.elementAt(i)).getSapId();
        				//String sapid= (String)((ContactSAPDetails)contactVect.elementAt(i)).getCustomerId();
    ...
    //getting device contacts list
  	...
  	private void gettingContacts(){
		try {
			...
			for (int i = 0; i < contactVect.size(); i++) {
				String sapid = (String)((ContactSAPDetails)contactVect.elementAt(i)).getSapId();
    ...
    private void gettingSelectedContacts(){
    	try{
    		if(listView != null){
    			...
    			int chksiz = contactVect.size();
        		for(int j = 0; j < contactVect.size(); j++){
        			boolean getstatus = status.get(j).booleanValue();
					String name = (String)((ContactSAPDetails)contactVect.elementAt(j)).getContactName();
					String id = (String)((ContactSAPDetails)contactVect.elementAt(j)).getContactDeviceId();
					String sapid = (String)((ContactSAPDetails)contactVect.elementAt(j)).getSapId();
					String cusid = (String)((ContactSAPDetails)contactVect.elementAt(j)).getCustomerId();
					String fname = (String)((ContactSAPDetails)contactVect.elementAt(j)).getFName();
					String lname = (String)((ContactSAPDetails)contactVect.elementAt(j)).getLName();
					String orgname = (String)((ContactSAPDetails)contactVect.elementAt(j)).getOrgName();
					
					/*String cusid = (String)((ContactSAPDetails)contactVect.elementAt(j)).getSapId();
    				String sapid= (String)((ContactSAPDetails)contactVect.elementAt(j)).getCustomerId();*/    	    		

					ContactSAPDetails obj = new ContactSAPDetails(name, id, sapid, cusid, fname, lname, orgname);
        			if(getstatus == true){	
    					ContactsConstants.showLog("get Selected name : "+name);
    					ContactsConstants.showLog("get Selected id : "+id);
    					ContactsConstants.showLog("get Selected sapid : "+sapid);
    					ContactsConstants.showLog("get Selected cusid : "+cusid);
    					ContactsConstants.showLog("get Selected fname : "+fname);
    					ContactsConstants.showLog("get Selected lname : "+lname);
    					ContactsConstants.showLog("get Selected orgname : "+orgname);
    					if(!SapGenConstants.selContactIdArr.contains(sapid)){
    					...
    					if(SapGenConstants.selContactIdArr.contains(sapid)){        					
        					if(cusid != null){
        	            		contactsArrySelctdList.add(sapid);
        	            	}
    					}
        			}
        			else{
        				if(SapGenConstants.selContactIdArr.contains(sapid)){
        					int index = findIndex(SapGenConstants.selContactIdArr, sapid);
        					SapGenConstants.selContactIdArr.remove(sapid);
        					if(index != -1){
            					SapGenConstants.selContactVect.remove(index);
        					}
        				} 
        			}
        		}	
    ...
    private void showActCrtScreen(){
    	try{
        	...
        	if(categoryAct != null){
	        	intent.putExtra("actobj", categoryAct);	        		
        	}  
    	...
    	try {
    		if(SapGenConstants.selContactVect != null && SapGenConstants.selContactVect.size() > 0){
    			for (int i = 0; i < SapGenConstants.selContactVect.size(); i++){
		    		String sapid = (String)((ContactSAPDetails)SapGenConstants.selContactVect.elementAt(i)).getSapId();
		    		String cusid = (String)((ContactSAPDetails)SapGenConstants.selContactVect.elementAt(i)).getCustomerId();
	    			String id = (String)((ContactSAPDetails)SapGenConstants.selContactVect.elementAt(i)).getContactDeviceId();	
		                    ...
		                    for (int j = 0; j < propsCount; j++) {
		                        if(j > 2){
		                            ...
		                            if(docTypeStr.equalsIgnoreCase("ZGSXCAST_CMPNYCNTCTDATA01")){
	                					...
	                					ContactSAPDetails obj = new ContactSAPDetails(name, id, sapid, cusid, fname, lname, orgname);
	                					SapGenConstants.selContactVect.set(pos, obj);
		                            }	  
		                        }
		                        else if(j == 0){
    ...
    public class ContactListAdapter extends BaseAdapter {
        ...
        public View getView(final int position, View convertView, ViewGroup parent) {
            ...
            try {
				String name = (String)((ContactSAPDetails)contactVect.elementAt(position)).getContactName();
	            String cname = "";
				int namefind = name.indexOf("                    ");
				if(namefind != -1){
				    String name_append = name.substring(0,namefind);
				...
				String oname = (String)((ContactSAPDetails)contactVect.elementAt(position)).getOrgName();
				ContactsConstants.showLog("oname :"+oname);
				if(oname != null && oname.length() > 0){
					holder.orgname.setVisibility(View.VISIBLE);
					holder.orgname.setText(oname);
				}else{
					holder.orgname.setVisibility(View.GONE);
				}

            	CheckBox cbox = (CheckBox) convertView.findViewById(R.id.cbox);   
				String sapid = (String)((ContactSAPDetails)contactVect.elementAt(position)).getSapId();
				String cusid= (String)((ContactSAPDetails)contactVect.elementAt(position)).getCustomerId();
				ContactsConstants.showLog("sapid :"+sapid);
				...
				ContactsConstants.showErrorLog("Error in ContactListAdapter : "+qw.toString());
			}
    ...
    OnItemClickListener listItemClickListener = new OnItemClickListener() {
		public void onItemClick(AdapterView<?> parent, View view, final int position, long id) {
			try{
				CheckBox box = (CheckBox)view.findViewById(R.id.cbox);
                boolean state = status.get(position).booleanValue();
                ContactsConstants.showLog("state on item click:"+state);
                ContactsConstants.showLog("position on item click:"+position);
                ...
                String name = (String)((ContactSAPDetails)contactVect.elementAt(position)).getContactName();
				String id1 = (String)((ContactSAPDetails)contactVect.elementAt(position)).getContactDeviceId();
				String sapid = (String)((ContactSAPDetails)contactVect.elementAt(position)).getSapId();
				String cusid = (String)((ContactSAPDetails)contactVect.elementAt(position)).getCustomerId();
				String fname = (String)((ContactSAPDetails)contactVect.elementAt(position)).getFName();
				String lname = (String)((ContactSAPDetails)contactVect.elementAt(position)).getLName();
				String orgname = (String)((ContactSAPDetails)contactVect.elementAt(position)).getOrgName();

				//String cusid = (String)((ContactSAPDetails)contactVect.elementAt(position)).getSapId();
				//String sapid= (String)((ContactSAPDetails)contactVect.elementAt(position)).getCustomerId();

				ContactSAPDetails obj = new ContactSAPDetails(name, id1, sapid, cusid, fname, lname, orgname);
    			if(!state){	
					ContactsConstants.showLog("Selected name : "+name);
					ContactsConstants.showLog("Selected id : "+id1);
					ContactsConstants.showLog("Selected sapid : "+sapid);
					ContactsConstants.showLog("Selected cusid : "+cusid);
					ContactsConstants.showLog("Selected fname : "+fname);
					ContactsConstants.showLog("Selected lname : "+lname);
				...
				ContactsConstants.showErrorLog("Error in listItemClickListener:"+dee.toString());
		    }
		}
    };
    
    public int findIndex(ArrayList array, String value) {
    	int index = -1;
    	try{
